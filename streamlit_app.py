...
if os.path.exists(TEMPLATE_PATH):
    with open(TEMPLATE_PATH, "rb") as f:
        saved = pickle.load(f)
        st.session_state.golden_templatebroad_template = saved["bytes"]
...
if results_file and plate_file and sample_file and st.session_state.get("golden_locked"):
    ...
    for sheet_name in wb.sheetnames:
        if "raw" in sheet_name.lower() and "data" in sheet_name.lower():
            ...
            break
    if ws is None:
        st.error(f"Could not find 'Raw data' sheet. Available: {wb.sheetnames}")
        st.stop()
    ...
    # FIXED: Safe mass loading (no re-reading file)
    if "samples" in globals() and "Desired mass in rxn (ng)" in samples.columns:
        ...
        mass = mass_values.mode().iloc[0] if not mass_values.mode().empty else 60
        ws["BA47"] = mass
    else:
        ws["BA47"] = 60
    ...
    # FIXED: Safe preview grid
    try:
        with openpyxl.load_workbook(output, read_only=True) as temp_wb:
            real_sheet = next(n for n in temp_wb.sheetnames if "raw" in n.lower() and "data" in n.lower())
        final_grid = pd.read_excel(output, sheet_name=real_sheet, usecols="BD:BO", skiprows=32, nrows=8, header=None)
        ...
        st.caption("Preview grid not shown — download Excel for full results")
